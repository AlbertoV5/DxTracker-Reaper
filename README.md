...
Everything related to Reaper and comparing Utterance is really efficient and it is done within seconds, what takes time is reading the guide track and saving the UEF into an array, and mostly depends on the configuration on Resemblyzer, CPU/GPU options and what not. For example, it took around 38 minutes to process a 4 hour, 44.1khz 16bit podcast with the hop, frame = 1, 3 setting with a 3.7 GHz Quad-Core Intel Xeon E5 CPU. Performance may vary, so go make a coffee in the mean time or watch some NBA highlights in your phone.
