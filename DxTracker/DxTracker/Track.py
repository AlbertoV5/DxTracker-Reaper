...
def ReadAudio(filePath, start, end):
    ...
    audio = audio[:,0] + audio[:,1] / audio.size
    return audio, sr
...
def CompareUtterance(DxUE, guideTrackUE, threshold = 0.91): #gtUF as a [position, guide track utterance] array
    scalarProduct = np.array([ 
        [guideTrackUE[i][0], np.dot(DxUE,guideTrackUE[i][1])] 
        for i in tqdm(range(len(guideTrackUE)))])
...
def CompareUtteranceExclusive(dxUE, gtUEF, threshold = 0.9): # Compare all speakers with guide track at once
    ...
    for i in range(len(gtUEF)):
        product = [np.dot(j, gtUEF[i][1]) for j in dxUE]
        higher = max(product)
        speaker = int(product.index(higher))
        
        scalarProductExclusive.append([gtUEF[i][0], higher, speaker])
