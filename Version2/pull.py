...
rscPath = reaper.GetResourcePath() + '/'
...
if smi > 1:
    ...
    dxData = [[i.trackName, i.sourcePath, i.offset, i.offset + i.itemLength, i.sourceSR] for i in items]
    ...
    for i in range(len(dxData)):
        config['DX' + str(i+1)] = {'name':dxData[i][0],'path':dxData[i][1],'start':dxData[i][2],'end':dxData[i][3],'sr':dxData[i][4]}
    ...
    reaper.ShowConsoleMsg('Not enough items selected')